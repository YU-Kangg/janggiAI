...
# Keep the ADB server alive after this launcher exits. Some automated shells
# terminate an adb server that was implicitly started by a short-lived command.
Start-Process -FilePath $adb -ArgumentList @('nodaemon', 'server') -WindowStyle Hidden | Out-Null
...
Write-Output ('App launched and USB server forwarding is active.' + $(if ($SkipInstall) { ' APK installation was skipped.' } else { '' }))
