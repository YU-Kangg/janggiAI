...
func run() -> void:
	...
	check(not app.game_mode_tabs.visible, "secondary setup controls initially collapsed")
	app.toggle_game_setup()
	check(app.game_mode_tabs.visible and app.game_setup_toggle.text == "대국 설정 접기", "new game setup expandable")
	app.toggle_game_setup()
	app.show_game_screen()
	check(not app.review_tabs.visible and not app.game_setup_toggle.visible and app.game_actions.visible and app.home_button.visible, "play screen hides home review and setup controls")
	...
	check(app.worst_move_ply() > 0 and not app.worst_move_button.disabled, "worst move navigation available")
	var worst_ply: int = app.worst_move_ply()
	app.show_worst_move()
	await wait_idle()
	check(app.review.moves.size() == worst_ply, "worst move navigation")
	...
	check(app.key_move_status.text.contains("핵심 장면 1/1") and app.key_move_status.text.contains("현재 수"), "key move position and current detail display")
	...
	check(app.key_move_status.text.contains("핵심 장면 1/2") and app.key_move_status.text.contains("손실"), "previous key move position and loss display")
