...
등록 클래스는 `JanggiNative`(RefCounted)입니다. Android 빌드 중에만 `janggi.gdextension.example`을 프로젝트에 복사하고,
내보내기가 끝나면 제거합니다. Android 전용 확장을 평소 데스크톱 실행에서 자동 로딩하지 않기 위한 처리입니다.
